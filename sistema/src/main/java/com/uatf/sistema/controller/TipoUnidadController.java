...
public class TipoUnidadController {
    ...
    @GetMapping
    public ResponseEntity<List<TipoUnidadDTO>> controller_get_all(){
        return ResponseEntity.ok(service.service_get_all());
    }

    @GetMapping("/{id}")
    public ResponseEntity<TipoUnidadDTO> controller_find_by_id(@PathVariable UUID id){
        return ResponseEntity.ok(service.service_find_by_UUID(id));
    }

    @PostMapping
    public ResponseEntity<TipoUnidadDTO> controller_save(@RequestBody TipoUnidadDTO dto){
        return ResponseEntity.status(HttpStatus.CREATED).body(service.service_save(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<TipoUnidadDTO> controller_update(@PathVariable UUID id, @RequestBody TipoUnidadDTO dto){

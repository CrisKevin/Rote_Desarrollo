...
public class ObservacionController {
    ...
    @GetMapping
    public ResponseEntity<List<ObservacionDTO>> controller_get_all(){
        return ResponseEntity.ok(service.service_get_all());
    }

    @GetMapping("/{id}")
    public ResponseEntity<ObservacionDTO> controller_find_by_id(@PathVariable UUID id){
        return ResponseEntity.ok(service.service_find_by_UUID(id)); 
    }

    @PostMapping
    public ResponseEntity<ObservacionDTO> controller_save(@RequestBody ObservacionDTO dto){
        return ResponseEntity.status(HttpStatus.CREATED).body(service.service_save(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ObservacionDTO> controller_update(@PathVariable UUID id, @RequestBody ObservacionDTO dto){

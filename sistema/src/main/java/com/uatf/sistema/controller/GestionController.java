...
public class GestionController {
    ...
    @GetMapping
    public ResponseEntity<List<GestionDTO>> controller_get_all(){
        return ResponseEntity.ok(service.service_get_all());
    }

    @GetMapping("/{id}")
    public ResponseEntity<GestionDTO> controller_find_by_id(@PathVariable UUID id){
        return ResponseEntity.ok(service.service_find_by_UUID(id));
    }

    @PostMapping
    public ResponseEntity<GestionDTO> controller_save(@RequestBody GestionDTO dto){
        return ResponseEntity.status(HttpStatus.CREATED).body(service.service_save(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<GestionDTO> controller_update(@PathVariable UUID id, @RequestBody GestionDTO dto){

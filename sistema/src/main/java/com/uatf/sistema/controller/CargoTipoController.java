...
public class CargoTipoController {
    ...
    @GetMapping
    public ResponseEntity<List<CargoTipoDTO>> controller_get_all(){
        return ResponseEntity.ok(service.service_get_all());
    }

    @GetMapping("/{id}")
    public ResponseEntity<CargoTipoDTO> controller_find_by_id(@PathVariable UUID id){
        return ResponseEntity.ok(service.service_find_by_UUID(id));
    }

    @PostMapping
    public ResponseEntity<CargoTipoDTO> controller_save(@RequestBody CargoTipoDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.service_save(dto));
    }
    
    @PutMapping("/{id}")
    public ResponseEntity<CargoTipoDTO> controller_update(@PathVariable UUID id, @RequestBody CargoTipoDTO dto){

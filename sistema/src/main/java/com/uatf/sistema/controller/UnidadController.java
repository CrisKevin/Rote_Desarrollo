...
public class UnidadController {
    ...
    @GetMapping
    public ResponseEntity<List<UnidadDTO>> controller_get_all(){
        return ResponseEntity.ok(service.service_get_all());
    }

    @GetMapping("/{id}")
    public ResponseEntity<UnidadDTO> controller_find_by_id(@PathVariable UUID id){
        return ResponseEntity.ok(service.service_find_by_UUID(id));
    }

    @PostMapping
    public ResponseEntity<UnidadDTO> controller_save(@RequestBody UnidadDTO dto){
        return ResponseEntity.status(HttpStatus.CREATED).body(service.service_save(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<UnidadDTO> controller_update(@PathVariable UUID id, @RequestBody UnidadDTO dto){

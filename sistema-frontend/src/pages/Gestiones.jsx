...
export default function Gestiones() {
  ...
  const cargarItems = async () => {
    ...
    const { data, error: errorMsg } = await gestionService.getAll();
  ...
  const abrirModalNuevo = () => {
    ...
    setFormData({ gestion: '', descripcion: '' });
    setErrorFormulario('');
  ...
  const confirmarEliminar = async () => {
    ...
    const { error } = await gestionService.eliminar(itemAEliminar.id);
  ...
  const itemsFiltrados = items.filter((item) => {
    ...
    return (
      item.gestion.toLowerCase().includes(terminoBusqueda) ||
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Gestiona todos los años académicos del sistema
          </p>
        </div>
        <button 
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4">
        <div className="flex items-center gap-2">
          ...
          <input
            type="text"
            placeholder="Buscar por gestión o descripción..."
            value={searchTerm}
      ...
      <ModalConfirmacion 
        ...
        mensaje={`¿Estás seguro de eliminar la gestión "${itemAEliminar ? itemAEliminar.gestion : ''}"? Esta acción no se puede deshacer.`}

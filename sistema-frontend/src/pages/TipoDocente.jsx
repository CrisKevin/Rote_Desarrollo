...
export default function TipoDocente() {
  ...
  // Función para cargar tipos de docente
  const cargarItems = async () => {
    setCargando(true);
    setError('');
    
    const { data, error: errorMsg } = await tipoDocenteService.getAll();
  ...
  const confirmarEliminar = async () => {
    ...
    const { error } = await tipoDocenteService.eliminar(itemAEliminar.id);
  ...
  <div className="space-y-6">
    <div className="flex justify-between items-start">
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Gestiona todos los tipos de docente del sistema
        </p>
      </div>
      <button 
    ...
    <ModalConfirmacion 
      ...
      titulo="Eliminar Tipo de Docente"

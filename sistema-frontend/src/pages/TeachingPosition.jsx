...
export default function TeachingPosition() {
  ...
  const cargarItems = async () => {
    ...
    const { data, error: errorMsg } = await teachingPositionService.getAll();
  ...
  const confirmarEliminar = async () => {
    ...
    const { error } = await teachingPositionService.eliminar(itemAEliminar.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <ModalConfirmacion 
        ...
        titulo="Eliminar"
        mensaje={`¿Estás seguro de eliminar "${itemAEliminar ? itemAEliminar.cargo : ''}"? Esta acción no se puede deshacer.`}

// src/pages/TipoPeriodo.jsx
import { Search, Edit, Trash2, Plus } from 'lucide-react';
import { useState, useEffect, useRef } from 'react';
import { tipoPeriodoService } from '../services/tipoPeriodoService.js';
import ModalFormulario from '../components/ModalFormulario';
import ModalConfirmacion from '../components/ModalConfirmacion';

export default function TipoPeriodo() {
  ...
  // Función para cargar tipos de período
  const cargarItems = async () => {
    setCargando(true);
    setError('');
    
    const { data, error: errorMsg } = await tipoPeriodoService.getAll();
  ...
  const guardarItem = async () => {
    ...
    if (itemEditando) {
      const { error } = await tipoPeriodoService.actualizar(itemEditando.id, datosEnviar);
      if (!error) {
        await cargarItems();
        cerrarModal();
      } else {
      ...
      const { error } = await tipoPeriodoService.crear(datosEnviar);
      if (!error) {
        await cargarItems();
        cerrarModal();
      } else {
  ...
  const confirmarEliminar = async () => {
    ...
    const { error } = await tipoPeriodoService.eliminar(itemAEliminar.id);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Gestiona todos los tipos de período del sistema
          </p>
        </div>
        <button 
      ...
      <ModalConfirmacion 
        ...
        titulo="Eliminar Tipo de Período"
        mensaje={`¿Estás seguro de eliminar el tipo de período "${itemAEliminar ? itemAEliminar.tipo : ''}"? Esta acción no se puede deshacer.`}
      ...
      <ModalFormulario 
        ...
        titulo={{
          nuevo: 'Nuevo Tipo de Período',
          editando: 'Editar Tipo de Período'
        }}
        campos={[
          { name: 'tipo', label: 'Tipo', placeholder: 'Ej: S0/1, S0/2, I/2025...', required: true },
          { name: 'descripcion', label: 'Descripción', placeholder: 'Descripción del tipo de período' }
        ]}
      />
    </div>
  );
